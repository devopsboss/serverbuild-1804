...
class nodejs::install::npm::frontail (
  ...
  # * see https://github.com/mthenw/frontail
  #
  if ($install != false) {
    ...
    #
    if ($install == true) {
      $install_command = 'npm install -g -s frontail'
    } else {
      # if $install isn't boolean then it's a version
      $version = $install
      $install_command = "npm install -g -s frontail@$version"
    }
    exec { 'install-frontail':
      path    => '/usr/bin',
      command => "sudo $install_command",
      creates => "$bin_folder/frontail",
      user    => 'root',
      require => Exec['npm-config-prefix'],
    }
    #
    # * Permissions
    # * /usr/local/bin/frontail ubuntu:www-data 750
    #
    file { "$bin_folder/frontail":
      owner   => $owner,
      group   => $group,
      mode    => $mode,
      require => Exec['install-frontail']
    }
  }

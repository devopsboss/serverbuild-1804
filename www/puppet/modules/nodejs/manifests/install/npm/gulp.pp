...
class nodejs::install::npm::gulp (
  ...
  if ($install != false) {
    ...
    #
    if ($install == true) {
      $install_command = 'npm install -g -s gulp'
    } else {
      # if $install isn't boolean then it's a version
      $version = $install
      $install_command = "npm install -g -s gulp@$version"
    }
    exec { 'install-gulp':
      path    => '/usr/bin',
      command => "sudo $install_command",
      creates => "$bin_folder/gulp",
      user    => 'root',
      require => Exec['npm-config-prefix']
    }
    #
    # * Permissions
    # * /usr/local/bin/gulp ubuntu:www-data 750
    #
    file { "$bin_folder/gulp":
      owner   => $owner,
      group   => $group,
      mode    => $mode,
